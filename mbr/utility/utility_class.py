...
class UtilityFunction:
    """
    Base class for utility functions used in Minimum Bayes Risk decoding.

    This abstract class defines the interface for utility functions that compute
    similarity scores between hypotheses and references, which are used in MBR decoding.
    """
    ...
    def compute_similarity(
        self, hyp: Iterable, ref: Iterable, src: Optional[Any] = None
    ) -> List[float]:
        """
        Compute similarity scores between hypotheses and references.

        Args:
            hyp: Iterable of hypothesis strings.
            ref: Iterable of reference strings.
    ...
    def compute_score_matrix(
        self, samples: List[str], src: Optional[Any] = None
    ) -> np.ndarray:
        """
        Compute a similarity matrix between all pairs of samples.

        Args:
            samples: List of text samples to compare.
            src: Optional source input that may be required by some similarity functions.
...
class SACREBLEU(UtilityFunction):
    ...
    def compute_similarity(self, hyp, ref, src):
        scores = [
            self.similarity.compute(predictions=[hyp[i]], references=[ref[i]])["score"]
            / 100.0
            for i in range(len(hyp))
        ]
        return scores
...
class SFR(UtilityFunction):
    ...
    def compute_similarity(self, hyp, ref, src):
        ...
        scores = []
        for i in range(len(hyp)):
            # TODO: Batch processing would be more efficient yet it takes more
            # GPU memory...
            hi = [hyp[i]]
            ri = [ref[i]]
